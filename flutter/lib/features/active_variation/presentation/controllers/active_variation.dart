...
class ActiveVariationController {
  ...
  changeActive({String activeName = "PETR4"}) async {
    finances.value = null;
    prices.value = null;
    await getFinances(activeName: activeName);
  }
  ...
  Future getFinances({String activeName = "PETR4"}) async {
    ...
    for (var data in listaDeDatas.asMap().entries) {
      ...
      Either financesEntity =
          await _getVariationsUseCase(activeName, timestramp, timestramp2);
